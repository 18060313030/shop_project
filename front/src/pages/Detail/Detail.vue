          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="items,index in spuSaleAttrList" :key="index">
                ...
                <!-- 商品的属性选项按钮 -->
                <dd changepirce="0" :class="{active:parseInt(item.isChecked)}" v-for="item,i in items.spuSaleAttrValueList" :key="item.id"
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 购买的商品数量 -->
                <input autocomplete="off" class="itxt" v-model="skuNum" @keyup.enter="goodsNumHandler" @blur="goodsNumHandler">
...
export default {
  ...
  methods: {
    ...
    // 用户输入的商品总数量进行约束，只能是大于等于1的正整数
    goodsNumHandler() {
      let sum = this.skuNum * 1 // 任何数值*1都为数字类型，如果转换不了数字，就为NaN
      if (isNaN(sum) || sum < 1) {
        this.skuNum = 1
      } else {
        this.skuNum = parseInt(this.skuNum) // 用户有可能输入浮点数,需要通过parseInt进行转换
      }
    },
    ...
    async addOrUpdateShopCar() {
      try {
        ...
        const res = await this.$store.dispatch('detail/addOrUpdateShopCart', {skuId: parseInt(this.$route.params.id), skuNum: this.skuNum})
        // 添加购物车成功，路由跳转
        // 【注意点】使用本地会话存储和query参数传递下个页面需要的数据
        // 本地存储存储商品数据对象，query存储商品数量
  ...
  computed: {
    ...
    // 63集 21分钟
    skuImageList() {
      // skuImageList：放大镜位置的图片数据 是一个数组格式
  ...
  created() {
    // 发起请求获取数据
    this.$store.dispatch('detail/getDetailData', this.$route.params.id)
  },
}
</script>
