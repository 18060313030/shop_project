<template>
  ...
  <div class="clearfix selector">
    ...
    <div class="type-wrap" v-for="(items,index) in attrsList" :key="items.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(item,index) in items.attrValueList" :key="index" @click="arrtValueHandler(items,item)">
            <a>{{item}}</a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    // 点击了品牌--发送数据给Search组件进行拼接
    tradeMarkHandler(data) {
      // 将参数传递到Search组件上
      this.$emit('revTradeMark', data)
    },

    // 点击了商品属性选项--发送数据给Search组件进行拼接
    arrtValueHandler(data, value) {
      const attrInfo = `${data.attrId}:${value}:${data.attrName}`
      this.$emit('revAttrInfo', attrInfo)
    },
  },
}
</script>
